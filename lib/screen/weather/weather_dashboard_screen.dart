...
import 'package:weatherapp/model/api/weather_api3.dart';
import 'package:weatherapp/model/api/weather_api_merge.dart';
...
class BandungWeather extends StatefulWidget {
  @override
  _BandungWeatherState createState() => _BandungWeatherState();
}

class _BandungWeatherState extends State<BandungWeather> {
  String _temperature = '';
  String _weather = '';

  Future<void> _fetchData() async {
    final response = await http.get(Uri.parse('https://data.bmkg.go.id/DataMKG/MEWS/DigitalForecast/DigitalForecast-JawaBarat.xml'));
    final document = xml.parse(response.body);
    final temperatureNode = document.findAllElements('temperature').firstWhere((node) => node.getAttribute('id') == 'Bandung');
    final weatherNode = document.findAllElements('weather').firstWhere((node) => node.getAttribute('id') == 'Bandung');
    setState(() {
      _temperature = temperatureNode.getAttribute('value');
      _weather = weatherNode.getAttribute('description');
    });
  }
...
import 'package:weatherapp/utils/weather_helper.dart';
                ...
                children: [
                  ...
                  Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                          windDirection!['timerange'][index]['value'][1]['\$t']
                              .toString(),
                          style: const TextStyle(
                              fontSize: 14, color: Colors.white)),
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              parserIconWheather(dataWeather!['timerange']['value']['\$t'], 35),
              const SizedBox(height: 13),
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
